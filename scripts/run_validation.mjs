import fs from 'fs';
import { OfflineDataProvider } from '../frontend/services/offlineDataProvider.js';
...
// Simple LocalStorageService shim so offlineDataProvider works without full browser
const TestRunner = {
  results: [],
  bugs: [],
  ...
  clearData() {
    ...
    global.localStorage.setItem('erp_auth_token', JSON.stringify({username:'admin', role:'admin'}));
  }
};
...
async function runPhase2BSuite() {
  ...
  await TestRunner.runSuite('Purchase Return Workflow', async () => {
    TestRunner.clearData();
    const product = { id: 'P-1', name: 'Cement', stock: 100, avgCost: 350, isActive: true };
    OfflineDataProvider.saveProduct(product);
    const dealer = { id: 'D-1', name: 'Supplier', outstanding: 5000, isActive: true };
    OfflineDataProvider.saveDealer(dealer);
  ...
  await TestRunner.runSuite('Real Shop Simulation (100 txns)', async () => {
    ...
    for(let i=0; i<100; i++) {
      ...
      if (type === 0 || type === 1) {
        ...
        OfflineDataProvider.saveSalesInvoice({
          ...
          totalAmount: 10000, paidAmount: (type === 0 ? 10000 : 2000), paymentMethod: 'Cash'
        });
        ...
        OfflineDataProvider.savePurchaseInvoice({
          ...
          totalAmount: 17000, paidAmount: 0
        });
        ...
        OfflineDataProvider.saveSalesReturn({
          ...
          totalAmount: 5000, refundAmount: 0, paymentMethod: 'Adjustment'
        });
        expectedCustomerOutstanding -= 5000;
        expectedStock += 5;
      } else if (type === 4) {
        // Expense
        // expenses are saved as erp_expenses
        const expenses = JSON.parse(global.localStorage.getItem('erp_expenses') || '[]');
        expenses.push({ id: `EXP-${i}`, amount: 1000, paymentMethod: 'Cash', date: today });
        global.localStorage.setItem('erp_expenses', JSON.stringify(expenses));
        expectedCash -= 1000;
      }
    }
    ...
    // total stock: P-2..P-5 = 4000. P-1 starts at 1000.
  ...
  TestRunner.results.forEach(r => console.log(`[${r.status}] ${r.suite}: ${r.error || 'OK'}`));
  
  if (TestRunner.bugs.length > 0) {
    console.log(`Found ${TestRunner.bugs.length} Bugs!`);
    fs.writeFileSync('PHASE2B_BUGS.json', JSON.stringify(TestRunner.bugs, null, 2));
  } else {
    console.log('All tests passed! Data Integrity Maintained.');
  }
}
